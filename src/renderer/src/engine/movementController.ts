...
export function startMovementController(): () => void {
    ...
    async function tick(): Promise<void> {
        if (stopped) return
        const { current, direction, setDirection, verticalDirection, setVerticalDirection } =
        ...
        if (current === 'walk') {
            ...
            const { atTop, atBottom } = await window.api.moveByY(verticalDirection * SPEED)
            if (atTop) setVerticalDirection(1)
            else if (atBottom) setVerticalDirection(-1)
        }
